...
async function renderPricing() {
  publicContent.innerHTML = pageShell("公开价格", "按目标平台清晰计费", "创建订单即扣费并开始收码；30 分钟未收到验证码自动退款。", `<div class="public-empty">正在加载价格…</div>`);
  try {
    ...
    publicContent.innerHTML = pageShell("公开价格", "按目标平台清晰计费", "创建订单即扣费并开始收码；30 分钟未收到验证码自动退款。", `<div class="pricing-grid">${cards || `<div class="public-empty">暂未启用可申请平台</div>`}</div>`);
  } catch (error) {
  ...
  publicContent.innerHTML = pageShell(
    ...
    `<div class="docs-layout">
      ...
      <div class="docs-content">
        ...
        <section id="services"><h2>服务列表</h2><p>获取当前可申请的目标平台。公开服务接口只返回平台代码、名称、说明、允许的邮箱类型、价格和有效期；库存与邮箱地址仅在登录后的申请页面按需展示。</p><pre class="public-code">${escPublic(serviceExample)}</pre><p>响应示例：</p><pre class="public-code">${escPublic(serviceResponse)}</pre></section>
        <section id="create"><h2>创建订单</h2><p>只提交目标平台代码，系统会自动从合规邮箱池原子分配一个邮箱。不能指定邮箱地址或邮箱供应商。</p><pre class="public-code">${escPublic(createExample)}</pre><p>成功响应：</p><pre class="public-code">${escPublic(responseExample)}</pre><p><code>request_id</code> 用于幂等创建；同一用户重复提交相同值不会重复扣款。</p></section>
