...
function renderDocs() {
  return pageHead("API 文档", "使用 Bearer API Key 调用订单与 Webhook 接口。") + `<div class="card"><div class="table-wrap"><table><thead><tr><th>方法</th><th>路径</th><th>用途</th></tr></thead><tbody>${[["GET","/api/v1/services","分页获取目标平台"],["POST","/api/v1/orders","创建注册订单"],["GET","/api/v1/orders/{id}","查询订单"],["POST","/api/v1/orders/{id}/submitted","确认已提交注册"],["POST","/api/v1/orders/{id}/complete","完成订单"],["POST","/api/v1/orders/{id}/cancel","取消并退款"],["GET","/api/v1/webhook-deliveries","分页查询 Webhook 投递"]].map(row => `<tr><td><code>${row[0]}</code></td><td><code>${row[1]}</code></td><td>${row[2]}</td></tr>`).join("")}</tbody></table></div><div class="card-body"><pre class="code-sample">curl -H 'Authorization: Bearer hm_...' '${location.origin}/api/v1/orders?page=1&amp;page_size=20'</pre></div></div>`;
}
...
async function loadAdmin() {
  ...
  if (["admin-pools", "admin-channels"].includes(state.view)) state.pools = rememberPage("pools", await api(`/api/v1/admin/mailbox-pools?page=${requestedPage("pools")}&page_size=20`));
...
function showServiceEditor(serviceID = "") {
  ...
  document.querySelector("#secret-modal")?.remove();
  document.body.insertAdjacentHTML("beforeend", `<div id="secret-modal" class="modal-backdrop"><div class="modal"><div class="card-head"><h2>${service.id ? "编辑" : "新建"}目标平台</h2><button class="icon-btn" data-action="close-modal" title="关闭">×</button></div><div class="card-body form-grid"><input id="service-id" type="hidden" value="${esc(service.id)}"><label>平台代码<input id="service-code" class="field" value="${esc(service.code)}" placeholder="github"></label><label>平台名称<input id="service-name" class="field" value="${esc(service.name)}"></label><label>说明<input id="service-description" class="field" value="${esc(service.description)}"></label><label>允许邮箱供应商<input id="service-providers" class="field" value="${esc((service.allowed_providers || []).join(", "))}" placeholder="outlook, hotmail"></label><label>发件人域名<input id="service-senders" class="field" value="${esc((service.sender_domains || []).join(", "))}" placeholder="example.com"></label><label>主题关键词<input id="service-subjects" class="field" value="${esc((service.subject_keywords || []).join(", "))}" placeholder="verification, 验证码"></label><label>验证码正则<input id="service-regex" class="field" value="${esc(service.regex)}"></label><div class="form-columns"><label>单价<input id="service-price" class="field" type="number" min="0" step="0.01" value="${Number(service.price)}"></label><label>有效期（秒）<input id="service-ttl" class="field" type="number" min="60" max="86400" value="${Number(service.ttl_seconds)}"></label></div><label class="check-label"><input id="service-enabled" type="checkbox" ${service.enabled ? "checked" : ""}> 启用该目标平台</label><button class="primary-btn" data-action="save-service">保存平台与收码规则</button></div></div></div>`);
}

const splitList = value => String(value || "").split(",").map(item => item.trim().toLowerCase()).filter(Boolean);
async function saveService() {
  const payload = { id: document.querySelector("#service-id")?.value, code: document.querySelector("#service-code")?.value.trim(), name: document.querySelector("#service-name")?.value.trim(), description: document.querySelector("#service-description")?.value.trim(), enabled: document.querySelector("#service-enabled")?.checked, allowed_providers: splitList(document.querySelector("#service-providers")?.value), price: Number(document.querySelector("#service-price")?.value), ttl_seconds: Number(document.querySelector("#service-ttl")?.value), sender_domains: splitList(document.querySelector("#service-senders")?.value), subject_keywords: splitList(document.querySelector("#service-subjects")?.value), regex: document.querySelector("#service-regex")?.value.trim() };
...
async function requestUpgrade() { const token = document.querySelector("#upgrade-token")?.value; if (!token) return toast("请输入升级令牌"); await api("/api/v1/admin/system/upgrade", { method: "POST", headers: { "X-HeroMail-Update-Token": token } }); toast("升级任务已提交，服务将短暂重启"); startUpgradePolling(); }
