...
const statusMap = { created: ["待分配", "orange"], allocating: ["分配中", "blue"], assigned: ["等待提交", "orange"], waiting_code: ["收码中", "blue"], code_received: ["已收码", "green"], completed: ["已完成", "green"], canceled: ["已取消", "red"], expired: ["已超时", "orange"], expired_refunded: ["已超时退款", "orange"], allocation_failed: ["分配失败", "red"], disputed: ["申诉中", "orange"], pending: ["待支付", "orange"], paid: ["已支付", "blue"], active: ["正常", "green"], disabled: ["已停用", "red"], available: ["可用", "green"], leased: ["租用中", "blue"], cooldown: ["冷却中", "orange"], auth_error: ["授权异常", "red"], blocked: ["已隔离", "red"], verified: ["已验证", "green"], pending_verification: ["待验证", "orange"], failed: ["失败", "red"], idle: ["等待任务", "blue"], backing_up: ["备份中", "orange"], queued: ["已排队", "orange"], updating: ["升级中", "orange"], success: ["成功", "green"] };
...
const mailboxProviderLabels = { outlook: "Outlook", outlook_de: "Outlook.de", hotmail: "Hotmail", gmail: "Gmail", icloud: "iCloud", mailcom: "Mail.com" };
const mailboxProviders = Object.entries(mailboxProviderLabels);
function providerLabel(provider) { return mailboxProviderLabels[provider] || provider || "未知"; }
...
function showMailboxRegistrationEditor(mailboxID) {
  ...
  if (!mailbox) return;
  const registered = new Set(mailbox.registered_platforms || []);
  const rows = state.services.map(service => {
    const done = registered.has(service.code);
...
async function markMailboxServiceRegistered(mailboxID, serviceID) {
  const button = document.querySelector(`[data-action="mark-mailbox-service-registered"][data-mailbox="${CSS.escape(mailboxID)}"][data-service="${CSS.escape(serviceID)}"]`);
  if (button) { button.disabled = true; button.textContent = "正在标记…"; }
  try {
    await api(`/api/v1/admin/mailboxes/${encodeURIComponent(mailboxID)}/services/${encodeURIComponent(serviceID)}/registered`, { method: "POST" });
    const mailbox = state.mailboxes.find(item => item.id === mailboxID);
    const service = state.services.find(item => item.id === serviceID);
    if (mailbox && service && !(mailbox.registered_platforms || []).includes(service.code)) {
      mailbox.registered_platforms = [...(mailbox.registered_platforms || []), service.code].sort();
    }
    await render();
...
async function importMailboxes() {
  if (state.busyAction === "mailbox-import") return;
  const file = document.querySelector("#mailbox-file")?.files?.[0];
  const text = document.querySelector("#mailbox-import-text")?.value || "";
  if (file && text.trim()) return toast("请只选择文件或粘贴文本其中一种方式");
  if (!file && !text.trim()) return toast("请选择文件或粘贴邮箱文本");
  const form = new FormData();
  if (file) form.append("file", file);
  else form.append("file", new Blob([text], { type: "text/plain" }), "pasted-mailboxes.txt");
  const button = document.querySelector('[data-action="import-mailboxes"]');
  state.busyAction = "mailbox-import";
  if (button) { button.disabled = true; button.textContent = "正在导入并验证…"; }
  try {
    ...
    state.busyAction = "";
    const currentButton = document.querySelector('[data-action="import-mailboxes"]');
    if (currentButton) {
      currentButton.disabled = false;
      currentButton.textContent = "导入并开始验证";
    }
  }
}
...
function renderAdminChannels() { return pageHead("接入渠道", "Microsoft 和 Google 使用 OAuth 授权；iCloud、Mail.com 仍使用加密保存的 IMAP 凭证。") + `<div class="admin-grid"><div class="card"><div class="card-head"><h2>Microsoft Graph OAuth2</h2></div><div class="card-body"><div class="status-list"><div class="status-item"><i class="status-dot"></i> OAuth 授权流程 <span class="status-ok">服务端验证</span></div><div class="status-item"><i class="status-dot"></i> Token 存储 <span class="status-ok">AES-256-GCM</span></div><div class="status-item"><i class="status-dot"></i> Graph 失败回退 <strong style="margin-left:auto">IMAP</strong></div></div></div></div><div class="card"><div class="card-head"><h2>Google Gmail OAuth2</h2></div><div class="card-body form-grid"><button class="primary-btn" data-action="google-oauth">连接 Google 邮箱</button><div class="notice">在 Google 官方页面完成登录和 2FA，授权后自动加入邮箱池。系统只保存加密 Token，不接收 Google 密码。</div></div></div><div class="card"><div class="card-head"><h2>IMAP 渠道</h2></div><div class="card-body"><div class="status-list"><div class="status-item"><i class="status-dot"></i> Gmail <strong style="margin-left:auto">imap.gmail.com:993</strong></div><div class="status-item"><i class="status-dot"></i> iCloud <strong style="margin-left:auto">imap.mail.me.com:993</strong></div><div class="status-item"><i class="status-dot"></i> Mail.com <strong style="margin-left:auto">imap.mail.com:993</strong></div></div><div class="notice">Gmail 也可使用应用专用密码；连接成功会自动扫描历史邮件。普通 Google 密码和 2FA 地址不能直接用于 IMAP。</div></div></div><div class="card"><div class="card-head"><h2>连接 Microsoft 邮箱</h2></div><div class="card-body form-grid"><button class="primary-btn" data-action="microsoft-oauth">连接 Microsoft 邮箱</button><div class="notice">OAuth 完成后自动识别 Outlook、Outlook.de 或 Hotmail，并加入唯一邮箱池。</div></div></div></div>`; }
